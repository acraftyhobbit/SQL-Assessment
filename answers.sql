...
SELECT models.name as model_name, brands.name as brand_name,
headquarters
...
SELECT COUNT(*) FROM models JOIN brands on models.brand_id = brands.brand_id WHERE brands.name = 'Ford';
...
SELECT * FROM models ORDER BY year ASC LIMIT 10 OFFSET 14;
...
SELECT b.name, m.year, m.name FROM brands AS b JOIN models as m ON b.brand_id = m.brand_id
ORDER BY b.name;
...
SELECT models.brand_id FROM models GROUP BY models.brand_id HAVING COUNT(models.model_id) > 5;
...
from models
join brands on brands.founded = models.year
) as m;